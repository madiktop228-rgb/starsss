...
from bot.core.utils.state import AdminTemplateStates
import json
...
@admin_required
async def cmd_admin_panel(event: Message | CallbackQuery, config: Config):
    text = "⚙️ Админ-панель:"
...
async def admin_manage_tasks(callback: CallbackQuery, config: Config, session: AsyncSession):
    ...
    # --- Формируем текстовое представление списка --- 
    tasks_list_lines = []
    if tasks_with_counts:
        for task, count in tasks_with_counts:
            ...
            # ----------------------------------------------
            tasks_list_lines.append(f"{status_icon} #{task.id} - {escaped_display_name} ({count} вып.)") # Используем escaped_display_name
        tasks_list_text = "\n".join(tasks_list_lines)
        message_text = f"📄 Список всех заданий:\n\n{tasks_list_text}"
    else:
        message_text = "📄 Заданий пока нет."
    # ---------------------------------------------

    # --- Клавиатуру оставляем для навигации и добавления --- 
    reply_markup = kb.admin_tasks_list_keyboard(tasks_with_counts)
...
async def process_distribution_hours(message: Message, config: Config, state: FSMContext):
    try:
        ...
        if hours < 1 or hours > 168:
            raise ValueError("Количество часов должно быть от 1 до 168")
        
        await state.update_data(distribution_hours=hours)
        user_data = await state.get_data()
        await show_task_confirmation(message, state, user_data)
        
    except ValueError as e:
        logger.warning(f"Admin {message.from_user.id} entered invalid hours '{message.text}': {e}")
        await message.answer(
            f"❌ Ошибка: {e}. Пожалуйста, введите число от 1 до 168.",
            reply_markup=kb.cancel_state_keyboard()
        )
...
async def show_task_confirmation(message_event: Message | CallbackQuery, state: FSMContext, data: dict, edit: bool = False):
    ...
    -------------------------------------
    <b>Описание:</b> {html.escape(desc[:100])}... # Экранируем HTML в описании
...
# --- Обработчик отмены для состояний создания шаблона ---
# Используем общий обработчик cancel_any_action, который ловит состояния AdminTemplateStates
# @router.callback_query(F.data == "cancel_state", AdminTemplateStates)
# async def cancel_template_action(callback: CallbackQuery, config: Config, state: FSMContext, session: AsyncSession):
#     # ... (код отмены, возврата в меню) ...
#     pass # Уже обрабатывается в cancel_any_action

# --- Конец блока создания шаблона ---

# --- Обработчики для Просмотра, Редактирования, Удаления ---
# (Ваши существующие обработчики для просмотра, редактирования, удаления)
# ...

# --- Обработчик отмены (должен ловить и NewsletterStates, и AdminTemplateStates) ---
# @router.callback_query(F.data == "cancel_state", StateFilter(AdminTemplateStates, NewsletterStates)) # Ловим во всех состояниях
# async def cancel_any_action(callback: CallbackQuery, config: Config, state: FSMContext, session: AsyncSession):
#     current_state = await state.get_state()
#     logger.info(f"Admin {callback.from_user.id} cancelled action from state {current_state}")
...
#     if current_state and current_state.startswith("AdminTemplateStates"):
#         # Если отмена была из состояний шаблона, возвращаемся в меню шаблонов
#         await templates_menu_handler(callback, session)
#     elif current_state and current_state.startswith("NewsletterStates"):
#          # Если отмена была из состояний рассылки, возвращаемся в меню рассылки
#          await newsletter_menu(callback, state) # Предполагаем, что есть такая функция
#     else:
#         # По умолчанию возвращаемся в главное меню админки
#         try:
#             await callback.message.edit_text(
#                 "Возврат в главное меню.",
#                 reply_markup=kb.admin_main_keyboard()
...
#                  )
#              except Exception as e:
#                   logger.error(f"Failed to return to admin menu after cancel: {e}")

@router.callback_query(F.data.startswith("admin_user_subtract_stars_"), StateFilter(None)) 
@admin_required
async def handle_subtract_stars_request(callback: CallbackQuery, config: Config, state: FSMContext, session: AsyncSession):
    try:
        user_id_to_manage = int(callback.data.split("_")[-1])
        admin_id = callback.from_user.id
        logger.info(f"Admin {admin_id} initiated subtract stars request for user {user_id_to_manage}.")

        # Проверим, существует ли пользователь
        user = await db.get_user(session, user_id_to_manage)
        if not user:
            await callback.answer("❌ Пользователь не найден.", show_alert=True)
            # Попытаемся убрать кнопки из сообщения профиля
            try:
                await callback.message.edit_reply_markup(reply_markup=None)
            except TelegramBadRequest: pass
            return

        # Сохраняем ID пользователя в состояние FSM
        await state.update_data(manage_user_id=user_id_to_manage)

        # Запрашиваем сумму для списания
        await callback.message.edit_text(
            f"➖ Введите сумму ⭐️ для списания с баланса пользователя @{html.escape(user.username or 'Нет')} (ID: {user_id_to_manage}).\n"
            f"Текущий баланс: {user.balance:.2f}⭐️\n\n"
            f"<i>Введите положительное число (например, 5 или 2.5).</i>",
            reply_markup=kb.cancel_state_keyboard() # Добавляем кнопку отмены
        )
        await state.set_state(AdminManageUser.subtracting_stars) # Устанавливаем состояние ожидания
        await callback.answer()
        ...
        logger.error(f"Unexpected error handling subtract stars request for '{callback.data}': {e}", exc_info=True)
        await callback.answer("❌ Произошла непредвиденная ошибка.", show_alert=True)
        await state.clear() # Очищаем состояние при ошибке


# --- НОВЫЙ Обработчик ввода суммы для списания ---
@router.message(AdminManageUser.subtracting_stars, F.text)
@admin_required
async def process_subtract_stars_amount(message: Message, config: Config, state: FSMContext, session: AsyncSession):
    state_data = await state.get_data()
    user_id_to_manage = state_data.get('manage_user_id')
    admin_id = message.from_user.id

    if not user_id_to_manage:
        logger.error(f"Admin {admin_id} in state subtracting_stars but manage_user_id is missing.")
        await message.answer("❌ Произошла ошибка состояния. Пожалуйста, начните заново.", reply_markup=kb.admin_main_keyboard())
        await state.clear()
        return

    try:
        amount_to_subtract = float(message.text.replace(',', '.'))
        if amount_to_subtract <= 0:
            raise ValueError("Сумма списания должна быть положительной.")

        logger.info(f"Admin {admin_id} trying to subtract {amount_to_subtract} stars from user {user_id_to_manage}.")

        # Вызываем функцию для списания баланса
        success, new_balance = await db.subtract_balance(session, user_id_to_manage, amount_to_subtract)

        if success:
            await session.commit() # Коммитим изменения после успешного вызова db функции
            logger.info(f"Successfully subtracted {amount_to_subtract} stars from user {user_id_to_manage}. New balance: {new_balance}")
            await message.answer(f"✅ Успешно списано {amount_to_subtract:.2f}⭐️. Новый баланс: {new_balance:.2f}⭐️.")
            await state.clear()

            # Показываем обновленный профиль пользователя
            user = await db.get_user(session, user_id_to_manage)
            if user:
                profile_text = f"""
👤 <b>Профиль пользователя:</b> @{html.escape(user.username or "Нет")}
...
"""
                await message.answer(
                 ...
                 await message.answer("Не удалось перезагрузить профиль пользователя после списания.", reply_markup=kb.admin_main_keyboard())

        else:
            # Если db.subtract_balance вернула False (например, недостаточно средств)
            await session.rollback() # Откатываем изменения
            logger.warning(f"Failed to subtract {amount_to_subtract} stars from user {user_id_to_manage}. Reason: Insufficient funds or user not found.")
            await message.answer(
                f"❌ Не удалось списать {amount_to_subtract:.2f}⭐️. Возможно, у пользователя недостаточно средств или он не найден.",
                reply_markup=kb.cancel_state_keyboard() # Даем шанс отменить или ввести другую сумму
            )
            # Не очищаем состояние

    except ValueError:
        logger.warning(f"Admin {admin_id} entered invalid amount '{message.text}' for subtracting stars.")
        await message.answer(
            "❌ Ошибка: Введите корректное положительное число (например, 5 или 2.5).",
            reply_markup=kb.cancel_state_keyboard() # Даем шанс отменить или ввести другую сумму
        )
        # Не очищаем состояние
    except Exception as e:
        await session.rollback() # Откатываем на всякий случай
        logger.error(f"Error processing subtract stars amount for user {user_id_to_manage}: {e}", exc_info=True)
        await message.answer("❌ Произошла ошибка при списании средств.", reply_markup=kb.admin_main_keyboard())
