...
def calculate_required_referrals(stars_amount: int) -> int:
    """
    Рассчитывает количество рефералов, необходимых для вывода определенного количества звезд.
    Формула: количество звезд / 3
    Например: для вывода 25 звезд нужно 25/3 = 8.33 -> 9 рефералов (округление вверх)
    """
    import math
    return math.ceil(stars_amount / 3)
...
async def request_op(user_id, chat_id, first_name=None, language_code=None, is_premium=None, action="subscribe"):
    try:
        ...
        async with aiohttp.ClientSession(connector=conn) as session:
            ...
            logger.debug(f"Sending request to SubGram API: URL=https://api.subgram.ru/request-op/, data={data}")
            ...
            async with session.post('https://api.subgram.ru/request-op/', headers=headers, json=data) as response:
                ...
                # Если нет рекламодателей, считаем что проверка пройдена
                if response_json.get('code') == 404:
                    logger.warning(f'SubGram response for user {user_id}: {str(response_json)}')
                    return 'ok', 200, []  # Возвращаем статус 200, чтобы пропустить пользователя
                
                if not response.ok:
                    logger.error(f'SubGram API error for user {user_id}: status={response.status}, response={str(response_json)}')
                    return 'error', response.status, []

                logger.info(f"SubGram API success for user {user_id}: status={response_json.get('status')}, code={response_json.get('code')}, links_count={len(response_json.get('links', []))}")
        ...
        return 'ok', 200, []  # В случае ошибки тоже пропускаем пользователя
...
async def check_subscription_for_type(
    ...
    if local_check_failed:
        ...
        caption = f'''<b>⭐️ПОЛУЧАЙ ПО 2 ЗВЕЗДЫ ЗА ДРУГА⭐️</b>

Получай звезды за задания и приглашенных друзей,а также за розыгрыши.

<b><u>Чтобы активировать бота:</u></b>

<b>1️⃣ Подпишись на наших спонсоров (это займет 5 секунд).
2️⃣ Нажми «Проверить подписку ✅»</b>

<a href="https://t.me/FreeStarsNews1/6"><b>Наши отзывы</b></a>'''
        try:
...
async def check_two_stage_subscription(
    ...
    if channels_stage1_check:
        ...
        if not all_subscribed_stage1:
            logger.info(f"Function check: User {user_id} failed overall STAGE 1 check. Showing channels.")
        ...
        logger.debug(f"Function check: User {user_id} successfully passed STAGE 1 check loop.")
    else:
        logger.debug(f"Function check: No channels to check for STAGE 1 for user {user_id}. Skipping loop.")

    logger.info(f"Function check: User {user_id} PASSED STAGE 1 check (or no channels). Proceeding.")
    ...
    if channels_stage2_check:
        ...
        if not all_subscribed_stage2:
            logger.info(f"Function check: User {user_id} failed overall STAGE 2 check. Showing channels.")
            await show_subscription_channels(event, state, bot, session, stage=2, failed_channels=failed_channels_stage2)
            return False # Проверка не пройдена

        logger.debug(f"Function check: User {user_id} successfully passed STAGE 2 check loop.")
    else:
        logger.debug(f"Function check: No channels to check for STAGE 2 for user {user_id}. Skipping loop.")

    logger.info(f"Function check: User {user_id} PASSED STAGE 2 check (or no channels). All checks complete.")
...
В нашем боте можно бесплатно зарабатывать звёзды, мы вывели уже более 400 тысяч звёзд, посмотри - <b><a href="https://t.me/FreeStarspay">https://t.me/FreeStarspay</a></b>
...
💬 Наш чат: <b><a href="https://t.me/+_hJ32Jmtd-lhNDli">https://t.me/+_hJ32Jmtd-lhNDli</a></b>
'''
    ...
    if user.current_task_id:
        ...
        if potential_task and potential_task.is_active and potential_task.id not in {t.id for t in user.completed_tasks}:
            ...
            if (is_premium and req in ('all', 'premium_only')) or (not is_premium and req in ('all', 'non_premium_only')):
                ...
                logger.debug(f"User {user_id} continues with saved task {user.current_task_id} (Premium OK)")
            else:
    ...
    if not task_to_show:
        ...
        if user.current_task_id != next_task_id:
            ...
            try:
                ...
                # return  # УБРАНО, чтобы всегда доходить до SubGram/Flyer
        ...
        try:
            ...
            if sg_status in ["success", "warning"] and sg_links:
                    ...
                    if flyer_tasks and len(flyer_tasks) > 0:
                        ...
                        if next_task:
                            ...
                            if task_action == 'give boost':
                                task_details_message = f"""🎯 <b>Доступно задание!</b>\n\n📌 Перейдите по ссылке и проголосуйте за <b><a href=\"{next_task.get('link', '#')}\">канал</a></b>.\n\n<b>🏆 Награда: 0.25⭐️</b>"""
                            else:
                                task_details_message = f"""🎯 <b>Доступно задание!</b>\n\n📌 Подпишитесь на <b><a href=\"{next_task.get('link', '#')}\">канал</a></b> и не отписывайтесь в течение 7 дней.\n\n<b>🏆 Награда: 0.25⭐️</b>"""
                            keyboard = InlineKeyboardBuilder()
                            if task_action == 'give boost':
                                if 'link' in next_task:
                                    keyboard.button(text="🔗 Голосовать x3", url=next_task['link'])
                                ...
                                if 'link' in next_task:
                                    keyboard.button(text="🔗 Подписаться", url=next_task['link'])
                                keyboard.button(text="✅ Я выполнил", callback_data="flyer_task_complete")
                                keyboard.button(text="⏩ Пропустить", callback_data="flyer_task_skip")
                                keyboard.adjust(1)
                                markup = keyboard.as_markup()
                ...
                try:
                    ...
                    await bot.send_message(7631252818, f"Traffy задания: [ {traffy_tasks} ] Для пользователя {user_id}")
                    if traffy_tasks:
                        task = traffy_tasks[0]
                        ...
                        logger.info(f"Для пользователя {user_id} нет доступных невыполненных заданий Traffy")
                except Exception as e:
...
async def handle_traffy_task_complete(callback: CallbackQuery, state: FSMContext, bot: Bot, session: AsyncSession):
    ...
    await bot.send_message(7631252818, f"Traffy check для пользователя {user_id}, task_id {task_id}: {check}")
...
async def show_tasks(message: Message, session: AsyncSession, bot: Bot, state: FSMContext):
    ...
    task_selection_text = """🎯 <b>Выберите тип заданий:</b>

📅 <b>Ежедневные задания</b> - выполняйте каждый день и получайте бонусы!

🔥 <b>Обычные задания</b> - разнообразные задания для заработка звезд!

💡 <i>Выберите подходящий тип заданий и начинайте зарабатывать!</i>"""
...
async def process_task_complete(callback: CallbackQuery, session: AsyncSession, bot: Bot, state: FSMContext):
    ...
    if task_completed_now and next_task_needed:
        ...
        await db.mark_task_completed_for_user(session, user_id, task_id)
        logger.debug(f"Called mark_task_completed_for_user for task {task_id}")
...
async def back_to_main(message: Message, session: AsyncSession, config: Config):
    try:
...
async def back_to_main_callback(callback: CallbackQuery, session: AsyncSession, config: Config):
    try:
        # await callback.message.delete()
        user_id = callback.from_user.id
...
async def process_gift_selection(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    # Проверяем количество рефералов (50% от стоимости)
    required_referrals = calculate_required_referrals(gift["cost"])
    ...
    if user_referrals < required_referrals:
        await callback.answer(
            ...
            f"Для вывода {gift['cost']}⭐\nНужно: {required_referrals} рефералов\n"
    ...
    await callback.message.answer(
        ...
        f'✅ Рефералов: {user_referrals}/{required_referrals}\n\n'
...
ПЕРЕД ПОДТВЕРЖДЕНИЕМ ЗАЯВКИ НЕОБХОДИМО НАПИСАТЬ АДМИНИСТРАТОРУ </b><b>@managerstarstg</b><b> </b>для выполнения автоматического вывода.
...
async def withdraw_request(callback: CallbackQuery, session: AsyncSession, bot: Bot, state: FSMContext, config: Config, gift_processor=None):
    user_id = callback.from_user.id
...
async def handle_local_task_complete(callback: CallbackQuery, session: AsyncSession, bot: Bot, state: FSMContext):
    ...
    if task.check_subscription and task.channel_id_to_check:
        is_subscribed = await check_member_with_delay(bot, task.channel_id_to_check, callback.from_user.id)
        if not is_subscribed:
            await callback.answer("Подпишитесь на канал и попробуйте снова.", show_alert=True)
            return
    # ⚠️ Важно: теперь код продолжается для всех заданий, включая задания без проверки

    # Отметить задание как выполненное
    await db.mark_task_as_completed(session, callback.from_user.id, task_id)
    
    # Сохранить информацию о выполненном локальном задании для штрафов
    try:
        saved = await db.save_local_completed_task(
            session=session,
            user_id=callback.from_user.id,
            task_id=task_id,
        ...
        if saved:
            logger.info(f"Saved local completed task for user {callback.from_user.id}, task {task_id}, channel {task.channel_id_to_check}")
        else:
            logger.warning(f"Failed to save local completed task for user {callback.from_user.id}, task {task_id}")
    except Exception as e:
        logger.error(f"Error saving local completed task: {e}", exc_info=True)

    # Начисляем награду
    await db.add_balance(session, callback.from_user.id, task.reward)
    await session.commit()

    # Уведомление о начислении награды
    await callback.answer(f"Вы получили {task.reward:.2f}⭐️", show_alert=True)
    ...
    # Переходим к следующему локальному заданию
    task_to_show = await db.get_next_available_task(session, callback.from_user.id, False, None)
    if task_to_show:
        await send_task_message(callback.message.chat.id, callback.from_user.id, False, session, bot, state, user.first_name, user.language_code)
    else:
        # Если локальных заданий больше нет, проверяем SubGram
        sg_status, sg_code, sg_links = await request_op(
            callback.from_user.id,
            callback.message.chat.id,
            first_name=callback.from_user.first_name,
        ...
        if sg_links:
            await state.update_data(sg_links=sg_links, current_index=0)
            await show_subgram_channel(callback.message.chat.id, sg_links, 0, bot)
        else:
            await bot.send_message(callback.message.chat.id, "Все задания выполнены, проверьте не появились ли новые задания через 5 минут!", reply_markup=kb.earn_stars_task_again_keyboard())
            await state.clear()
...
async def handle_local_task_skip(callback: CallbackQuery, session: AsyncSession, bot: Bot, state: FSMContext):
    ...
    # Переходим к следующему локальному заданию
    task_to_show = await db.get_next_available_task(session, callback.from_user.id, False, task_id)
    if task_to_show:
        await send_task_message(callback.message.chat.id, callback.from_user.id, False, session, bot, state, user.first_name, user.language_code)
    else:
        # Если локальных заданий больше нет, проверяем SubGram
        ...
        if sg_links:
            ...
            await bot.send_message(callback.message.chat.id, "ℹ️ На данный момент доступных заданий для вас нет. Попробуйте зайти позже!", reply_markup=kb.earn_stars_task_again_keyboard())
            await state.clear()

# --- Вспомогательная функция для показа каналов (если ее нет) ---
# Эту функцию вызывает middleware и recheck_handler при неудаче
async def show_subscription_channels(
    ...
    session: AsyncSession,
    stage: int,
    failed_channels: Optional[List[Channel]] = None # Опционально: список проваленных каналов
):
    user = event.from_user
    is_premium = user.is_premium if user else False
    ...
    if stage == 1:
        ...
        check_type = 'start' # Первый этап всегда 'start'
        state_to_set = SubscriptionCheckStates.waiting_primary_check
        text = '''<b>⭐️ПОЛУЧАЙ ПО 2 ЗВЕЗДЫ ЗА ДРУГА⭐️</b>
...
<a href="https://t.me/FreeStarsNews1/6"><b>Наши отзывы</b></a>'''
        ...
        check_type = 'start' # Предполагаем, что второй этап тоже относится к старту
...
<a href="https://t.me/FreeStarsNews1/6"><b>Наши отзывы</b></a>'''
    ...
    if not channels_to_show:
        logger.info(f"No channels to show for stage {stage} for user {user.id}")
        # Возможно, стоит сбросить состояние, если каналов нет
        await state.clear()
        if isinstance(event, CallbackQuery):
            await event.answer("Нет каналов для проверки на данном этапе.", show_alert=True)
        return

    # --- Используем новую клавиатуру с этапом ---
    reply_markup = await kb.get_channels_keyboard(channels_to_show, check_type=check_type, stage=stage)

    message_object = event if isinstance(event, Message) else event.message
    if not message_object: return # Не можем отправить/отредактировать

    try:
        # Пытаемся отредактировать существующее сообщение, если это коллбэк
        if isinstance(event, CallbackQuery):
            await message_object.edit_text(text, reply_markup=reply_markup, disable_web_page_preview=True,)
        else: # Или отправляем новое, если это сообщение
            await message_object.answer(text, reply_markup=reply_markup, disable_web_page_preview=True,)
    except Exception as e:
        logger.debug(f"Could not edit/send subscription message for stage {stage}, user {user.id}: {e}. Sending new.")
        # Если редактирование не удалось, отправляем новое сообщение
        try:
            await bot.send_message(user.id, text, reply_markup=reply_markup)
            # Удаляем старое сообщение с кнопками, если это был коллбэк
            if isinstance(event, CallbackQuery) and event.message:
                 try: await event.message.delete()
                 except Exception: pass
        except Exception as e2:
            logger.error(f"Failed to send new subscription message for stage {stage}, user {user.id}: {e2}")

    # Устанавливаем состояние FSM
    await state.set_state(state_to_set)
    logger.info(f"Set state {state_to_set} for user {user.id}")
...
async def recheck_subscription_handler(callback: CallbackQuery, bot: Bot, session: AsyncSession, state: FSMContext, config: Config):
    ...
    # --- Логика после проверки ТЕКУЩЕГО этапа ---
    if not all_subscribed_current_stage:
        logger.info(f"User {user_id} FAILED recheck for stage {stage}.")
        await callback.answer(f"Вы не подписаны на все каналы! Пожалуйста, проверьте еще раз.", show_alert=True)
        # Показываем каналы текущего этапа снова
        await show_subscription_channels(callback, state, bot, session, stage, failed_channels=failed_channels_current_stage)
        return # Завершаем обработку

    # --- Если ТЕКУЩИЙ этап пройден успешно ---
    logger.info(f"User {user_id} PASSED recheck for stage {stage}.")

    if stage == 1:
        # Проверяем, есть ли каналы для второго этапа
        channels_stage2_exists = await db.get_filtered_second_stage_channels_all(session, is_premium)
        if channels_stage2_exists:
            ...
            # Показываем каналы второго этапа
            # Эта функция сама отредактирует сообщение или удалит старое и отправит новое
            await show_subscription_channels(callback, state, bot, session, stage=2)
            # --- УДАЛЯЕМ ЭТУ СТРОКУ ---
            # # Удаляем сообщение с кнопками первого этапа (НЕ НУЖНО, т.к. show_subscription_channels это делает)
            # try: await message.delete()
            # except Exception: pass
            # -------------------------
            return # Завершаем, ждем проверки второго этапа
        else:
            # Если второго этапа нет, считаем проверку полностью пройденной
            logger.info(f"User {user_id} passed stage 1, no stage 2 found. All checks complete.")
            # Переходим к финальной части (как будто пройден этап 2)
            pass # Продолжаем выполнение кода ниже
...
📌 Подпишитесь на <b><a href="{next_task.get('link', '#')}">канал</a></b> и проголосуйте за него.
...
<b>🏆 Награда: 0.25⭐️</b>"""
                    ...
                    if 'link' in next_task:
                        keyboard.button(text="🔗 Перейти в канал", url=next_task['link'])
